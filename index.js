...
function getMapsInJourney(directions) {
    ...
    var sortedMapIds = mapsSortedByElevation.filter(
        (map) => mapIds.indexOf(map.id) !== -1
    );
    return sortedMapIds;
}
    ...
    if (startNode != null && endNode != null) {
        startNode.directionsTo(
            ...
            function (error, directions) {
                ...
                endNode.directionsTo(
                    startNode,
                    { accessible: false, directionsProvider: 'offline' },
                    function (error, reverseDirections) {
...
function onDataLoaded() {
    ...
    for (var j = 0, jLen = locations.length; j < jLen; ++j) {
        ...
        for (var k = 0, kLen = locationPolygons.length; k < kLen; ++k) {
            ...
            if (
                oldLocation == null ||
                oldLocation.sortOrder > location.sortOrder
            ) {
    ...
    mapsSortedByElevation = venue.maps.sort(
...
function drawExitMarkers() {
	//mapView.createMarker('<img src="css/green-exit-marker" />', );
}
